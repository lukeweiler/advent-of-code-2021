...
fn main() {
    ...
    let mut just_called = 0;
    for call in calls {
        for board in &mut boards {
            board.update(call);
            if board.check_bingo() {
                winning_boards += 1;
                println!("winning_boards: {}", winning_boards);
                if winning_boards == num_boards {
                    println!("Score for losing board: {}", board.score(call));
                }
                (*board)[0][0].number = 0xFFu8;
    ...
    // for board in boards {
    //     print_board(&board);
    //     if (board)[0][0].number != 0xFFu8 {
    //         println!("Score for horrible board: {}", board.score(just_called));
    //     }
    // }
}